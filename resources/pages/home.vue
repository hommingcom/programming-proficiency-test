<template>
    <div class="container px-3 flex flex-col mx-auto">
      <section class="mb-5 py-5 flex flex-col md:flex-row justify-around items-center">

        <div class="mb-2 md:mb-0 ">
          <div class="px-4 py-2 border-2 border-grey rounded-3xl relative">
            <label class="absolute -my-4 -mx-2 tracking-wider
            bg-white px-2 text-grey-darker">
              Filtro Propietario
            </label>
            <input type="text" name="owner" id="inputFilterOwner"
            class="appearance-none border-2 border-white rounded w-full py-2
            text-grey-darker leading-tight focus:outline-none focus:bg-white focus:border-white"
        ...
        <div class="mb-2 md:mb-0 ">
          <div class="px-4 py-2 border-2 border-grey rounded-3xl relative">
            ...
            class="appearance-none border-2 border-white bg-white rounded w-full py-2
            text-grey-darker leading-tight focus:outline-none focus:bg-white focus:border-white">
...
<div class="flex  flex-col md:flex-row">
        ...
        <div class="mb-2 md:mb-0">
          <div class="px-4 py-2 border-2 border-grey rounded-3xl relative">
            ...
            <input type="date" name="starDate" id="filterStartDate"
            class="appearance-none border-2 border-white rounded w-full py-2 bg-white
            text-grey-darker leading-tight focus:outline-none focus:bg-white focus:border-white"
...
<div class="">
  <button
  class="border-2 border-red-400 px-4 py-2 rounded-3xl hover:bg-red-500 hover:text-white
  font-bold tracking-wider hover:cursor-pointer"
  @click="resetFilters()">
    Borrar filtros
  </button>
</div>
  ...
  computed: {
    filteredData() {
      ...
      if (this.filters.rentedFrom !== null && this.filters.rentedFrom !== '') {
        data = data.filter((item) => item.rentedFrom > new Date(this.filters.rentedFrom));
      }
      if (this.filters.rentedTo !== null && this.filters.rentedTo !== '') {
        debugger;
        data = data.filter((item) => item.rentedTo < new Date(this.filters.rentedTo));
      }
